...
for findex, frames in tqdm(dataloader):
    ...
    for vidname, frame in frames.items():
        ...
        tracks, trksoff = tracker.update(dets)
...
movlist = ['NE_Moiz', 'NW_Serena', 'SW_Ethan']
mangall = loadPredefGal('tmpgal/')

for mov in movlist:
    convertdict = dict()
    with open(mov + 'tmp2.txt', "r") as rawdat:
        trktoimgdat = rawdat.read()
    trktoimgdat = trktoimgdat.split('\n')
    for line in tqdm(trktoimgdat):
        if line == '':
            continue
        parts = line.split(' ')
        reids = list()
        ...
        if parts[1] != '':
            ...
            if len(reids) > 0:
                convertdict[id] = mode(reids)[0][0]

    with open(mov + 'conv.txt', "w") as datfile:
        for key in convertdict:
            datfile.write(str(key) + ' ' + str(convertdict[key]) + '\n')
    ...
    outfile = np.loadtxt(mov + 'tmp.txt', delimiter=',')
    outfile[:, 1] = convertnp(outfile[:, 1])

    np.savetxt(mov + '.txt', outfile, delimiter=',', fmt='%d')
